...
def main():
    ...
    with train_tab:
        ...
        col2.dataframe(data=y_train.map(mapping), use_container_width=True)

    with test_tab:
        col1, col2 = st.columns([0.8, 0.2])
        col1.dataframe(data=X_test, use_container_width=True)
        col2.dataframe(data=y_test.map(mapping), use_container_width=True)
    ...
    report = ClassificationManager.evaluate_model(model_fit, X_test, y_test, mapping)

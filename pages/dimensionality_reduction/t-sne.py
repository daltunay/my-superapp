...
from src.statistics.dimensionality_reduction import TSNEManager
...
def main():
    ...
    st.subheader("t-SNE")
    with st.container(border=True):
        tsne_manager = TSNEManager(max_n_components=3)
        tsne_manager.set_model()
    ...
    try:
        st.plotly_chart(tsne_manager.scatter_3d_plot(), use_container_width=True)
    except ValueError:
        st.error("Number of principal components not sufficient for the plot")
